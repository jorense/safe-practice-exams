...
import styles from './PSM2ExamQuiz.module.css'
...
function PSM2ExamQuiz({ onGoHome, onGoBackToExam, numberOfQuestions = 30, autoShowExplanation = false, examMode = 'exam', includeSeenQuestions = true }) {
  ...
  const calculateTimerDuration = (questionCount) => {
    if (questionCount <= 30) {
      ...
      // Practice mode: 3 minutes per question (180 seconds each)
      return questionCount * 180
    } else {
  ...
  useEffect(() => {
    let availableQuestions = psm2Questions
    
    // Filter questions based on exam mode
    if (examMode === 'exam') {
      // Exam mode: only single-select questions (filter out multi-select)
      availableQuestions = psm2Questions.filter(q => !q.questionType || q.questionType !== 'multiple')
    }
